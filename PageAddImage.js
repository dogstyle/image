PAGE.extend(function(instance, proto, log) {
	...
	PAGE.wait("Constructors.DispatcherSpreader", function(DispatcherSpreader) {
				...
				if (instance.Images && instance.Images[url]) {
					...
					var interval2 = setInterval(function() {
						if ($image2.children().length > 0) {
							typeof callback === "function" && callback($image2, index, url)
			...
			dispatcher.makeRunner(random, function() {
				...
				var interval = setInterval(function() {
					if ($image.children().length > 0) {
						typeof callback === "function" && callback($image, index, url)
		...
		, addWaitSVGBatch = proto.Image.addWaitSVGBatch = function(urlArray, eachCallback, batchCallback) {
				...
				var length = (function() {
					var count = 0
					for (var x in urlArray) count++
					return count
				}())
					, complete = 0
					, returnedImages = new Array(length)
					, index = 0

				if (typeof urlArray !== "object" || !length) { return }

				for(var x in urlArray) {

					addWaitSVG(x, function($image, index, source) {
						returnedImages.splice(index, 1, $image)
						typeof eachCallback === "function" && eachCallback($image, index, source)
						complete++
						if (complete === length) {
